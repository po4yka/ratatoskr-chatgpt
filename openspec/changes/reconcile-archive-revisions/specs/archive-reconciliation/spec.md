...
When a conversation observed in an earlier snapshot is absent from a later
ConsumerExport snapshot, the service SHALL add a
`MissingFromLatestSnapshot` observation for that archive. It SHALL retain the
conversation, all of its earlier revisions, and its last present observation,
and it SHALL NOT emit or infer an explicit provider-deletion state.
