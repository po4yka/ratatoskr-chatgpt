...
> **Status:** authenticated archive receipt, hostile-input inspection, synthetic parsing, deterministic reconciliation, portable export, scoped privacy deletion, exact-version reparse, parser-migration reports, and read-only fixture admission are implemented. The service uses content-addressed BlobStore evidence and the single current `schema.sql`; archive-, conversation-, and tenant-scoped privacy deletion removes exclusive raw, extracted, and normalized evidence while atomically recording content-free audit and downstream tombstones. Reparse and parser migration only use verified preserved raw archives and genuinely registered newer compiled parsers, with side-effect-free dry runs and deterministic reports. The synthetic parser is not support for a real ChatGPT export: the documented owner-authorized fixture process and an admitted compiled-parser golden are required before a provider schema can be claimed. The receipt flow does not yet persist or publish full normalized reconciliation results. Not implemented yet: real-export parser support and Compliance adapters.
...
The private real-export discovery and minimized golden admission process is documented in
[`docs/testing/OWNER_FIXTURE_DISCOVERY.md`](docs/testing/OWNER_FIXTURE_DISCOVERY.md).
...
non-content warning code; they are not dropped or silently reparented.
...
The current reconciliation boundary produces a structured in-memory report;
the receipt pipeline does not yet persist it. A future fully evidenced report
may look like:
